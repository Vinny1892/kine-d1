...
**Solução validada:** transação diferida com CAS — leituras em autocommit, escritas num buffer, `Commit()` envia um `batch` precedido de uma guarda que aborta se o estado mudou. Desenho completo no [ADR-0002](adr-0002-deferred-transaction.md); medições em [`spikes/results/spike-6.md`](spikes/results/spike-6.md).
...
Quatro estratégias fazem round-trip byte a byte; a decisão está no [ADR-0001](adr-0001-blob-representation.md).
